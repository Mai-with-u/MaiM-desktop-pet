...
本文档详细列出了 pet.py 重构的所有任务，按照优先级和阶段进行组织。
...
### 1.1 创建新的配置系统
- [x] 创建 `config/` 目录
- [x] 创建 `config/schema.py` - 配置模式定义
- [x] 创建 `config/loader.py` - 配置加载器
- [x] 创建 `config/__init__.py` - 配置模块入口

### 1.2 更新配置 schema
- [x] 定义 `InterfaceConfig` - 界面配置模型
- [x] 定义 `RenderConfig` - 渲染配置模型
- [x] 定义 `Live2DConfig` - Live2D 配置模型
- [x] 定义 `AnimationConfig` - 动画配置模型
- [x] 定义 `PerformanceConfig` - 性能配置模型
- [x] 定义 `DatabaseConfig` - 数据库配置模型
- [x] 定义 `StateConfig` - 状态配置模型
- [x] 定义 `Config` - 主配置模型

### 1.3 实现配置加载器功能
- [x] 实现 `ensure_config_exists()` - 确保配置文件存在
- [x] 实现 `load_config()` - 加载配置文件
- [x] 实现 `get_scale_factor()` - 获取缩放倍率
- [x] 实现 `save_config()` - 保存配置文件
- [x] 实现 `update_config_value()` - 更新单个配置值

### 1.4 更新所有使用旧 config 的文件
- [x] 更新 `src/core/chat.py`
- [x] 更新 `src/core/router.py`
- [x] 更新 `src/frontend/pet.py`
- [x] 更新 `src/frontend/presentation/refactored_pet.py`
- [x] 更新 `src/frontend/core/managers/render_manager.py`
- [x] 更新 `main.py`
- [x] 修复 `src/frontend/bubble_speech.py`
- [x] 修复 `src/frontend/bubble_input.py`
- [x] 更新 `tests/test.py`

### 1.5 清理和测试
- [x] 删除旧的 `config.py` 文件
- [x] 创建 `test_config_simple.py` 测试脚本
- [x] 测试配置系统功能
- [x] 验证所有配置属性可以正确访问

### 1.6 文档
- [x] 创建 `docs/REFACTORING_SUMMARY.md` - 重构总结文档
- [x] 更新配置使用示例
